...
public class DeliveryServiceImplementation implements DeliveryServiceInterface {
        ...
        public Delivery acceptOrder(@NonNull UUID deliveryPersonId, @NonNull UUID orderId) {
                ...
                if (order.getStatus() != OrderStatus.READY) {
                        throw new RuntimeException(
                                        "La commande n'est pas prête pour la livraison. Statut actuel: "
                                                        + order.getStatus());
                }
         ...
         */
        private AvailableOrderDTO buildAvailableOrderDTO(Order order, BigDecimal userLatitude,
                        BigDecimal userLongitude) {
        ...
        @Override
        public Delivery autoAssignCourier(@NonNull UUID orderId) {
                Order order = orderRepository.findById(orderId)
                                .orElseThrow(() -> new RuntimeException("Order not found with ID: " + orderId));
                ...
                // 1. Trouver les livreurs de la même pharmacie qui sont actifs
                List<User> couriers = userRepository.findActiveDeliveryPersonsByPharmacy(order.getPharmacy().getId());

                // 2. Filtrer ceux qui n'ont pas de livraison en cours
                Optional<User> availableCourier = couriers.stream()
                                .filter(u -> deliveryRepository.findOngoingDeliveriesByPerson(u.getId()).isEmpty())
                                .findFirst(); // On prend le premier disponible pour l'instant

                if (availableCourier.isPresent()) {
                        User courier = availableCourier.get();
                        return assignDelivery(order.getPharmacy().getId(), orderId, courier.getId());
                }
